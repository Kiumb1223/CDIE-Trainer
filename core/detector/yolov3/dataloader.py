...
class V3Dataset(Dataset):
    def __init__(
        ...
        super(V3Dataset, self).__init__()
        
        with open(annotation_path, encoding='utf-8') as f:
            annotation_lines = f.readlines()
        self.annotation_lines   = annotation_lines
        self.input_shape        = input_shape          # (h, w, ...) 或 (h, w)
    ...
    def __getitem__(self, index):
        ...
        #---------------------------------------------------#
        #   训练时进行 Mosaic + MixUp 等随机增强
        #   验证时一般关闭 mosaic/mixup（初始化时设 False）
        #---------------------------------------------------#
        if (self.mosaic and self.train and
            self.rand() < self.mosaic_prob and
            self.epoch_now < self.epoch_length * self.special_aug_ratio):

            # 采样 3 行，加上当前这行，一共 4 张做 Mosaic
        ...
        #---------------------------------------------------#
        #   将 box 从 x1,y1,x2,y2 转为 cx,cy,w,h（归一化）
        #   保留 box[:, 4] 为类别
        #---------------------------------------------------#
        if len(box) != 0:
            # 归一化到 0~1
    ...
    def get_random_data(
        ...
        image_data = cv2.merge((
            cv2.LUT(hue_c, lut_hue),
            cv2.LUT(sat_c, lut_sat),
    ...
    def merge_bboxes(self, bboxes, cutx, cuty):
        ...
        for i in range(len(bboxes)):
            for box in bboxes[i]:
                ...
                tmp_box.extend([x1, y1, x2, y2, box[-1]])
    ...
    def get_random_data_with_Mosaic(
        ...
        for line in annotation_line:
            ...
            #---------------------------------#
            box = np.array(
                [np.array(list(map(int, b.split(','))))
                 for b in line_content[1:]]
            )
        ...
        #---------------------------------#
        r               = np.random.uniform(-1, 1, 3) * [hue, sat, val] + 1
        hue_c, sat_c, val_c = cv2.split(cv2.cvtColor(new_image, cv2.COLOR_RGB2HSV))
